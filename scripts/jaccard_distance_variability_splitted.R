...
# saveRDS(jc_matrix,'originalVar_edge_jc_mat_FORWARD.rds')
saveRDS(jc_matrix,'originalVar_edge_jc_mat_FORWARD_80.rds')
...
# saveRDS(jc_matrix,'originalVar_edge_jc_mat_BACKWARD.rds')
saveRDS(jc_matrix,'originalVar_edge_jc_mat_BACKWARD_80.rds')
...
edge_df_for_frequency <- as.data.frame(matrix(NA,length(edge_df_for[,1]),2))
edge_df_for_frequency[,1] <- row.names(edge_df_for)
edge_df_for_frequency[,2] <- rowMeans(edge_df_for)

pheatmap(edge_df_for, show_rownames = F)
...
edge_df_back_frequency <- as.data.frame(matrix(NA,length(edge_df_back[,1]),2))
edge_df_back_frequency[,1] <- row.names(edge_df_back)
edge_df_back_frequency[,2] <- rowMeans(edge_df_back)

edge_df_frequency <- as.data.frame(rbind(edge_df_for_frequency, edge_df_back_frequency))
names(edge_df_frequency) <- c("edgeIDs","frequency_percent")

write_csv(edge_df_frequency, "edge_df_frequency.csv")

pheatmap(edge_df_back, show_rownames = F)
